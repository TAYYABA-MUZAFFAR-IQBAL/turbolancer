...
def signup():
    if request.method == "POST":
        ...
        encoded_country = turbolancer_data_Security.encrypt(key, request.form['con'])
        d = 'd'
        skils = []
...
def signin():
    if request.method == "POST":
        ...
        if user:
            ...
            user_id = generate_id()
            user = {"_id": user_id, "email": encoded_email, "password": encoded_password}
            developer_collection.insert_one(user)
...
def insert_skill():
    if request.method == 'POST':
        ...
        skill_data = {"_id": skill_id, "skill": skill, "image": image,'req':req}
...
def insert_tech():
    if request.method == 'POST':
        ...
        skill_data = {"_id": skill_id, "sector": sec,"tech": tech, "image": image,'req':req}
...
def get_quiz_data(x):
    ...
    for topic in topics:
        ...
        selected_questions = filtered_data[:min(len(filtered_data), rounded_answer)]  # Select the first 40 questions or all available questions if there are fewer
        all_questions.extend(selected_questions)

    random.shuffle(all_questions)  # Shuffle all the selected questions from different topics
...
def selects_skill(email):
    if email:
        ...
        rendered_template = render_template_string(template_content,x=  'hi' ,h1 = 'Select your serving sector', function2 = 'next(this)',function1 = None, disc = 'Click to select tecknology for ', b = 'Select technology', o=encoded_skills, image = img, name = name,email2= turbolancer_data_Security.encrypt(key, email), email = email)

        return rendered_template
    else: return '<h1><b>Method Not Allowd</b></h1>'
...
def select_technology(email, topic):
    if email and topic:
        ...
        rendered_template = render_template_string(template_content, h1=f'Select technology for {topic}', function2=None, function1='select(this)', disc='Click to choose technology', b='Take test', o=encoded_skills, image=img, name=name, email=email)
...
def upload_image():
    if 'image' in request.files:
        ...
        developer_collection.update_one({"email": encoded_email}, {"$set": {"image": '/get_image/' + str(image_id)}})
...
def update_data():
    ...
    if encoded_email and text_area_value:
        ...
        if developer:
            developer_collection.update_one({"email": encoded_email}, {"$set": {"about_self": text_area_value}})
